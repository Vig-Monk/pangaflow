...
export async function getMerchantLocationHandler(
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> {
    try {
        const orgId = requireOrgId(req);
        const location = await storesService.fetchMerchantLocation(orgId);
...
export async function saveMerchantLocationHandler(
    req: Request,
    res: Response,
    next: NextFunction
): Promise<void> {
    try {
        const orgId = requireOrgId(req);
        const location = await storesService.saveMerchantLocation(
            orgId,
            req.body
        );
        success(res, location);
