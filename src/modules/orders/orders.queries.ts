...
// Database Queries — Storefront and Merchant Orders.
// =============================================================================
...
export async function getOrdersSummary(orgId: string): Promise<OrdersSummary> {
  const result = await query<{ today_count: string; today_revenue: string; pending_count: string }>(
    `SELECT 
 ...
 * Returns true if successfully updated; false if stock was insufficient.
 */
...
export async function getOrderById(orgId: string, orderId: string): Promise<OrderWithItems | null> {
