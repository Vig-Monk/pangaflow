...
async function runE2EIntegrationTest() {
    console.log(
        "🚀 Starting Soko Commerce End-to-End Integration Verification...\n"
    );
    ...
    try {
        ...
        const sigRes = await axios.post(
            `${API_BASE}/products/upload-signature?target=products`,
            {},
            authHeaders
        );
        ...
        if (!signature || !timestamp || !folder.includes(orgId)) {
            throw new Error(
                "Signature response was missing required fields or incorrectly scoped."
            );
        }
        ...
        const storeConfigRes = await axios.patch(
            `${API_BASE}/store`,
            {
        ...
        console.log(
            `   ✓ Store configured and Published. ID: ${seededStoreId}, Slug: ${storeSlug}`
        );
        ...
        const shoes = createdProducts.find(
            (p: any) => p.name === "Classic Running Shoes"
        );
        console.log(`   ✓ Bulk insertion completed. Product ID: ${shoes.id}`);

        console.log("6. [Customer] Retrieving public store metadata...");
        const pubStoreRes = await axios.get(
            `${API_BASE}/public/stores/${storeSlug}`
        );
        const pubStore = pubStoreRes.data.data;

        const forbiddenStoreKeys = [
            "id",
            "org_id",
            "status",
            "created_at",
        ...
        forbiddenStoreKeys.forEach(key => {
            if (key in pubStore)
                throw new Error(`Leaked store internal key: ${key}`);
        });
        console.log("   ✓ Whitelist mapping for store properties verified.");

        console.log(
            "7. [Customer] Simulating public checkout order placement..."
        );
        ...
        const checkoutRes = await axios.post(
            `${API_BASE}/public/stores/${storeSlug}/orders`,
            checkoutPayload
        );
        ...
        console.log(
            "8. [Merchant] Verifying dashboard sync & store_id mapping..."
        );
        const merchantOrdersRes = await axios.get(
            `${API_BASE}/orders`,
            authHeaders
        );
        ...
        if (!foundInLog) {
            throw new Error(
                "Newly created order was not synchronized in the merchant dashboard order log."
            );
        }
