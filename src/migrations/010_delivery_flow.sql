-- =============================================================================
-- Migration: 010_delivery_flow.sql (FIXED)
-- Soko Platform — Checkout & Delivery Flow Schema
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Migrate Existing Statuses & Update Constraint on `orders`
-- ---------------------------------------------------------------------------

-- Step A: Drop old constraint first so we can normalize existing data
ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_status_check;

-- Step B: Migrate any existing legacy 'fulfilled' rows to 'delivered'
UPDATE orders SET status = 'delivered' WHERE status = 'fulfilled';

-- Step C: Apply updated constraint supporting the full delivery progression
ALTER TABLE orders ADD CONSTRAINT orders_status_check
  CHECK (status IN ('pending', 'confirmed', 'assigned', 'out_for_delivery', 'delivered', 'cancelled'));

-- ---------------------------------------------------------------------------
-- 2. Add Delivery Metadata Columns to `orders`
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Merchant Locations (Fulfillment / Pickup Base)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Reference Estates & Localities (Zero-Cost Local Geocoding Table)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6. Seed Curated East African Estates & Localities
-- ---------------------------------------------------------------------------
